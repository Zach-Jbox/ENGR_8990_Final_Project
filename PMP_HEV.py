...
def calculate_SOC_dot(Tm, Wm, Tg, Wg, SOC):
    """
    Calculate the SOC_dot based on motor and generator power and battery parameters.
    
    Parameters:
        Tm (float): Motor torque (Nm)
        Wm (float): Motor speed (rad/s)
        Tg (float): Generator torque (Nm)
        Wg (float): Generator speed (rad/s)
        SOC (float): Current State of Charge
        
    Returns:
        SOC_dot (float): Rate of change of SOC
    """
    # Calculate Motor Power (positive for consumption, negative for regeneration)
    Pm = Tm * Wm
    
    # Calculate Generator Power
    Pg = Tg * Wg
    
    # Battery Power is the sum of motor and generator power and auxiliary power
    Pbatt = Pm + Pg + Paux
    
    # Battery voltage considering SOC and internal resistance
    Vbatt = Voc - R_batt * (SOC * Q_batt)
...
# Stacked Area Chart for Power Split
power_split_chart = go.Figure()

# Add engine power (stacked)
power_split_chart.add_trace(go.Scatter(
    x=df_speed.index, 
    y=df_speed['Engine Power (W)'], 
    mode='lines', 
    name='Engine Power',
    stackgroup='one'  # Creates stacking
))

# Add motor power (stacked)
power_split_chart.add_trace(go.Scatter(
    x=df_speed.index, 
    y=df_speed['Motor Power (W)'], 
    mode='lines', 
    name='Motor Power',
    stackgroup='one'  # Adds to the same stack
))

# Update layout
power_split_chart.update_layout(
    title="Power Split: Engine vs. Motor (Stacked Area Chart)",
    xaxis_title="Time Steps",
    yaxis_title="Power (W)",
    showlegend=True
)
...
# Show the chart
power_split_chart.show()

# Stacked Bar Chart for Power Split
power_split_bar_chart = go.Figure()

# Add engine power (stacked)
power_split_bar_chart.add_trace(go.Bar(
    x=df_speed.index, 
    y=df_speed['Engine Power (W)'], 
    name='Engine Power'
))
...
# Update layout
power_split_bar_chart.update_layout(
    barmode='relative',  # Enables stacking
    title="Power Split: Engine vs. Motor (Stacked Bar Chart)",
    xaxis_title="Time Steps",
    yaxis_title="Power (W)",
    showlegend=True
)
